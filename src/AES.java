public class Aes{
    ...
    /**
     * Inverse Galois table used for opMixColumns
     */
     ...
     */
    public static final int[] recon = { 0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a,
    ...
    private void copy2DArray(int[][] dest, int[][] src)
            {
            assert dest.length == src.length && dest[0].length == src[0].length;
    ...
    // Encryption methodes //
    private static int[][] sKey(int[][] keyMatrix, int start){
            int[][] state = new int[4][4];
            for (int i = 0; i < state.length; i++){
            for(int j = 0; j < state.length; j++){
            state[i][j] = keyMatrix[i][4 * start + j];
            }
            }
            return state;
            }
    ...
    public static void shiftRows(int[][] state){
            ...
            state[i] = rotateL(state[i], i);
            }
            }

    private static int[] rotateL(int[] state, int t){
    ...
    public static void mixColumns(int[][] state){
            ...
            int[][] tmpState = new int[4][4];
            for(int i = 0; i < tmpState.length; i++){
            System.arraycopy(state[i], 0, tmpState[i], 0, 4);
            }

            for(int i = 0; i < tmpState.length; i++){
            for(int j = 0; j < tmpState[0].length; j++){
            state[i][j] = computeMethod(tmpState, galois, i, j);
            }
            }
            }

    private static int computeMethod(int[][] state, int[][] gal, int i, int j){
            ...
            mixColSum ^= mixColCalc(a, b);
            }
            return mixColSum;
            }

    private static int mixColCalc(int a, int b){
            if(a == 1){
            return b;
            }else if (a == 2){
    ...
    public static int[][] keySch(String key){

            int keySize = key.length() * 4;
            int columnSize = keySize + 48 - (32 *((keySize / 64) - 2));
            int[][] keyMatrix = new int[4][columnSize / 4];
            int reconPointer = 1;
            int[] t = new int[4];
    final int counter = keySize / 32;
            int k;
            ...
            int kPoint = counter;
            while(kPoint < (columnSize / 4)){
            int tmp = kPoint % counter;
            if (tmp == 0){
            for(k = 0; k < 4; k++){
            ...
            while (kPoint < ktmp) {
            for (k = 0; k < 4; k++) {
            keyMatrix[k][kPoint] = keyMatrix[k][kPoint - 1] ^ keyMatrix[k][kPoint - counter];
            }
            kPoint++;
            }
            }
    ...
    public void opSubBytes(int[][] state) {
            for (int i = 0; i < state.length; i++) //Inverse Sub-Byte subroutine
            {
            for (int j = 0; j < state[0].length; j++) {
            int h = state[j][i];
            state[j][i] = invsbox[h / 16][h % 16];
            }
            }
            }
    public void opShiftRows(int[][] state) {
            for (int i = 1; i < state.length; i++) {
            state[i] = rotateR(state[i], i);
            }
            }

    private int[] rotateR(int[] state, int t) {
            if (state.length == 0 || state.length == 1 || t % 4 == 0) {
            return state;
            }
            while (t > 0) {
    ...
    public void opMixColumns(int[][] state) {
            int[][] tmpState = new int[4][4];
            for(int i = 0; i < 4; i++)
            {
            System.arraycopy(state[i], 0, tmpState[i], 0, 4);
            ...
            for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 4; j++) {
            state[i][j] = opComputeMethode(tmpState, invgalois, i, j);
            }
            }
            }

    private int opComputeMethode(int[][] state, int[][] gal, int i, int j)
            {
            int mixColSum = 0;
            for (int k = 0; k < 4; k++) {
            int a = gal[i][k];
            int b = state[k][j];
            mixColSum ^= opmixColCalc(a, b);
            }
            return mixColSum;
            }

    private static int opmixColCalc(int a, int b)
            {
            if (a == 9) {
